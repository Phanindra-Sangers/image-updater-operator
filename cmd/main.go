...
func main() {
	...
	flag.BoolVar(&enableWebhookReceiver, "enable-webhook-receiver", true,
	...
	// One workload reconciler per supported kind, sharing reconcile logic via the adapter.
	for _, adapter := range workload.Adapters() {
		if err := (&controller.WorkloadReconciler{
			Client:   mgr.GetClient(),
			Scheme:   mgr.GetScheme(),
			Recorder: mgr.GetEventRecorderFor("workload-" + adapter.Name),
			Adapter:  adapter,
		}).SetupWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create controller", "controller", "Workload", "kind", adapter.Name)
			os.Exit(1)
		}
	}
	...
	if enableWebhookReceiver {
		if err := registrywebhook.RegisterIndex(mgr); err != nil {
			setupLog.Error(err, "unable to register image repository index")
			os.Exit(1)
		}
		if err := mgr.Add(&registrywebhook.Receiver{
			Client: mgr.GetClient(),
